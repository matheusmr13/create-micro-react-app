...
program
  ...
  .option(
    ...
    ${chalk.black.bgWhite(' > cmra start -p https://my-app.xyz ')}
...
program
  ...
  .option(
    ...
    ${chalk.black.bgWhite(' > cmra build ./src/lib/index.js -l ')}
  `
  )
  .option(
    '-m, --microfrontend',
  ...
  .option(
    ...
    ${chalk.black.bgWhite(' > cmra build -a my-webapp && cmra build -p my-webapp ')}
  `
  )
  .option(
    '-c, --configuration-file <configuration_file>',
...
program
  ...
  .option(
    ...
    Creates a monorepo application with a webapp and microfrontend template (created with cra)

    ${chalk.black.bgWhite(' > cmra create my-app -a ')}
  `
  )
  .option(
    '-w, --webapp',
    `
    Creates a webapp container template to import some microfrontends (created with cra)

    ${chalk.black.bgWhite(' > cmra create my-app -w ')}
  `
  )
  .option(
    '-m, --microfrontend',
    `
    Creates a microfrontend to be used/imported by a webapp.

    ${chalk.black.bgWhite(' > cmra create my-microfrontend -m ')}

    Can be used beside --app parameter to create a webapp with an initial microfrontend like
    ${chalk.black.bgWhite(' > cmra create my-app -am ')}
  `
  )
  .option(
    '-l, --library',
    ...
    ${chalk.black.bgWhite(' > cmra create my-cool-library -l ')}
    ...
    ${chalk.black.bgWhite(' > cmra create cart -ml ')}
  ...
  .option(
    ...
    ${chalk.black.bgWhite(' > cmra create my-app -a --template typescript ')}
  `
  )
  .action((name, options) => {
    const opts = {
    ...
    if (options.webapp) types.push(create.TYPE.WEBAPP);
