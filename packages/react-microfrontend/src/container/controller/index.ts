...
class Controller {
  ...
  async setupAllMicrofrontends() {
    ...
    const setupMicrofrontend = (method) =>
      Promise.all(
        Object.values(this.microfrontends).map(async (micro: any) => {
          //   try {
  ...
  initialize() {
    ...
    fetchRetry(`./${microfrontendFolderName}/meta.json`, {
      ...
      this.microfrontends = Object.keys(meta).reduce(
        (agg, microfrontendName) =>
          Object.assign(agg, {
      ...
      this.call(
        CALLBACKS.MICROFRONTENDS_INFOS_DISCOVERED,
        this.getMicrofrontendsOnStatus(Microfrontend.STATUS.CREATED)
      );
